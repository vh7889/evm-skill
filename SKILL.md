...
这个 skill 用于 EVM 链批量与单钱包操作，当前包含 30 个脚本：
- 批量生成钱包
- 批量分发 gas 代币
- 批量归集 gas 代币
- 批量分发 ERC20 代币
...
- 单钱包一对一转 ERC20 代币
- 单钱包调用闭源合约
- 通过链上交易哈希提取 to/value/data（可编辑后再调用）
- 无 ABI 改参数（仅简单类型，需两步确认后广播）
...
使用 OKX 相关脚本前，需要先在 OKX OnchainOS 开发者门户申请 API 凭证：
- 申请地址：`https://web3.okx.com/zh-hans/onchainos/dev-portal`
...
- 方式1（直接复制交易）：`tx_hash + rpc` 提取 `to/value/data` 后，原样广播
- 方式2（有 ABI 改参数，推荐）：提供 ABI，解码参数 -> 修改 -> 重新编码 -> 广播
- 方式3（无 ABI 改参数）：仅支持简单静态类型；必须“先预览草案，再确认执行”

## 术语说明（便于协作者理解）

- `selector`：函数选择器，即 `data` 前 4 字节（例如 `0x095ea7b3`）
- 候选函数名：根据 selector 查询出来的“可能函数签名列表”，不一定唯一
- `best_guess`：脚本给出的最可能函数签名（仍可能误判）
- `confidence`：脚本对 `best_guess` 的置信度（仅作参考）
- 预览草案（preview）：只生成“修改前后对比”，不广播
- 确认执行（execute）：用户确认后，才允许广播

## 强制执行流程（必须）

所有“按哈希复用交易”的任务，必须按以下顺序执行，不能跳步：

1. 解析阶段（必须）
- 输入：`tx_hash + rpc`
- 输出：`to/value/data`
- 若 `to=null`（合约创建交易），立即停止并提示用户

2. 复制交易阶段（用户选择简单复制时）
- 使用 `to/value/data` 走 OKX `simulate -> broadcast -> orders`
- `simulate` 失败时禁止广播
- 必须回传：`orderId`、`txHash`、`txStatus`

3. 改参交易阶段（用户选择修改参数时）
- 先“解析并展示”，再“用户确认”，最后“广播”
- 必须先告知用户：
  - 可能函数名（selector 查询结果）
  - 最可能函数（`best_guess`）和置信度（`confidence`）
  - 可能参数（解码结果）
- 未经用户确认，不得直接改参上链

4. 简单类型与复杂类型分流（必须）
- 简单静态类型：可直接解码并给出“修改前/修改后参数 + 修改前/修改后 calldata”，用户确认后广播
- 复杂/动态类型：不得直接按单一结果执行
  - 必须展示候选函数列表并明确“可能误判”
  - 必须由用户选择最终函数或提供 ABI 后，才可继续

5. 安全确认（必须）
- 任何“改参广播”都必须有显式确认动作（yes/confirm-token 等）
- 无确认，一律禁止广播
...
- `scripts/single_call_by_txhash.py`
  - 通过交易哈希查询 `to/value/data`，导出可编辑 JSON 参数
- `scripts/single_edit_calldata_no_abi.py`
  - 无 ABI 条件下，按简单静态类型修改 calldata，先“预览草案”，后“确认执行”
- `scripts/query_selector_4byte.py`
  - 根据 4-byte selector 查询候选函数签名（4byte.directory）
...
## 12) 通过交易哈希提取 to/value/data（可编辑后再调用）
...
可选参数：
- `--output <路径>`：自定义输出 JSON 文件路径，不填则默认输出到 `output/log`
...
说明：
- 输出 JSON 可自行修改后，再配合其它脚本调用
- 若目标哈希是合约创建交易（`to=null`），脚本会报错并退出

## 13) 无 ABI 改参数（仅简单类型，两步确认）

第一步：预览草案（只生成确认文件，不广播）
...
python scripts/single_edit_calldata_no_abi.py preview \
  ...
  --set-items 1=<新地址> \
  --set-items 2=1000000000000000000
```

第二步：确认执行（必须 `yes + confirm-token`）

```bash
python scripts/single_edit_calldata_no_abi.py execute \
  --proposal-file <预览草案json> \
  --private-key <执行钱包私钥> \
  --rpc <RPC地址> \
  --confirm yes \
  --confirm-token <预览阶段输出token>
```

规则：
- 只支持简单静态类型：`address`、`uint*`、`int*`、`bool`、`bytes1..bytes32`
- 动态类型（`bytes/string/tuple/array`）不支持，直接拒绝修改
- 预览阶段必须输出并展示：修改前 calldata、修改后 calldata、具体参数变化
- 确认执行阶段未传 `--confirm yes` 或 token 不匹配，一律禁止广播

## 14) 根据 selector 查询函数签名（4byte）

命令：

```bash
python scripts/query_selector_4byte.py --selector 0xc2998238
```

说明：
- 该查询是候选集，不保证唯一与 100% 准确
- 可配合 `--calldata` 做兼容性校验并输出 `best_guess/confidence`
- 返回多个候选且分数接近时，仍需人工确认

## 15) 单钱包 write_contract（ABI + function + args）
...
## 18) 批量 write_contract（ABI + function + args）
...
## 22) 单钱包 write_contract（OKX Gateway 版）
...
## 24) 批量 write_contract（OKX Gateway 版）
...
## 26) OKX Onchain Gateway 全接口通用调用
...
## 29) OKX Wallet Portfolio 全接口通用调用
