...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="批量归集 Gas 代币（原生币）")
    parser.add_argument("--main-address", required=True, help="主钱包地址（接收地址）")
    parser.add_argument("--wallet-csv", required=True, help="分发钱包 CSV 文件（需包含私钥）")
...
def main() -> None:
    ...
    if args.log_csv:
        ...
        log_csv = log_dir / f"{wallet_csv.stem}_gas_collect_log.csv"
    ...
    def process_row(row: Dict[str, str]) -> None:
        ...
        try:
            ...
            balance = int(rpc.call(lambda w3: w3.eth.get_balance(from_addr)))
            gas_limit = 21000
            ...
            fee_budget = gas_limit * gas_price
            value = int(balance - fee_budget)
            fee_fields = {"gasPrice": gas_price}

            if value <= 0:
                row["状态"] = "FAILED"
                row["失败原因"] = "余额不足以覆盖 gas"
                row["时间"] = now_text()
                write_log(log_csv, rows, file_lock)
                return
            ...
            row["归集数量Wei"] = str(value)
            row["归集数量"] = str(rpc.call(lambda w3: w3.from_wei(value, "ether")))
            row["成功哈希"] = tx_hash
            row["时间"] = now_text()
