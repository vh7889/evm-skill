...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="批量分发 ERC20 代币")
    ...
    parser.add_argument("--amount", help="固定数量，例如 0.5")
    parser.add_argument("--amount-min", help="范围最小值，例如 0.1")
    parser.add_argument("--amount-max", help="范围最大值，例如 0.3")
...
def main() -> None:
    ...
    account = web3.eth.account.from_key(args.main_private_key)
    ...
    if args.log_csv:
        ...
        log_csv = log_dir / f"{wallet_csv.stem}_erc20_distribution_log.csv"
    ...
    nonce_cursor = web3.eth.get_transaction_count(from_addr, "pending")
    def next_nonce() -> int:
    ...
    def process_row(row: Dict[str, str]) -> None:
        ...
        amount_human = pick_amount(args)
        try:
            amount_base = to_base_units(amount_human, decimals)
        ...
        row["转出地址"] = from_addr
        row["接收地址"] = to_addr
        row["分发数量"] = str(amount_human)
        row["分发数量Base"] = str(amount_base)
        row["状态"] = "IN_PROGRESS"
        row["成功哈希"] = ""
        row["失败原因"] = ""
        row["时间"] = now_text()
