...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="批量查询原生 Gas 代币余额")
...
def load_or_init_output(output_csv: Path, wallets: List[Dict[str, str]]) -> List[Dict[str, str]]:
...
def main() -> None:
    ...
    if args.output_csv:
        ...
        output_csv = out_dir / f"{wallet_csv.stem}_gas_balance_query.csv"
    ...
    rows = load_or_init_output(output_csv, wallets)
    ...
    def process_row(row: Dict[str, str]) -> None:
        ...
        try:
            ...
            balance_wei = int(rpc.call(lambda w3: w3.eth.get_balance(addr)))
            balance = str(rpc.call(lambda w3: w3.from_wei(balance_wei, "ether")))
            row["余额Wei"] = str(balance_wei)
            row["余额"] = balance
            row["状态"] = "SUCCESS"
            row["失败原因"] = ""
            row["时间"] = now_text()
