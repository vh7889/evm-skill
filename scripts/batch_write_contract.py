...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="批量 write_contract 调用（ABI + function + args）")
...
def main() -> None:
    ...
    fn_args_text = json.dumps(fn_args, ensure_ascii=False)
    ...
    if args.log_csv:
        ...
        log_csv = log_dir / f"{wallet_csv.stem}_write_contract_log.csv"
    ...
    def process_row(row: Dict[str, str]) -> None:
        ...
        try:
            ...
            def build_tx_data() -> str:
                def _op(w3: Web3) -> str:
                    contract = w3.eth.contract(address=contract_addr, abi=abi)
                    fn_call = build_contract_function(
                        contract,
                        args.function,
