...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="批量 write_contract（OKX Gateway: simulate -> broadcast）")
...
def main() -> None:
    ...
    fn_args_text = json.dumps(fn_args, ensure_ascii=False, default=str)
    ...
    if args.log_csv:
        ...
        log_csv = log_dir / f"{wallet_csv.stem}_write_contract_okx_gateway_log.csv"
    ...
    rows = load_or_init_log(log_csv, wallets, contract_addr, args.function, fn_args_text)
    ...
    def process_row(row: Dict[str, str]) -> None:
        ...
        try:
            ...
            def build_tx_data_and_estimate() -> Dict[str, Any]:
                def _op(w3: Web3) -> Dict[str, Any]:
                    ...
                    tx_tmp = fn_call.build_transaction(
                        {
                            "chainId": chain_id,
                            "from": from_addr,
                            "nonce": nonce,
                    ...
                    return {"data": str(tx_tmp["data"]), "estimate": int(est)}

                return rpc.call(_op)

            tx_tmp = build_tx_data_and_estimate()
            tx_data = tx_tmp["data"]
