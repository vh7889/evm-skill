...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="单钱包调用闭源合约（OKX Gateway: simulate -> broadcast）")
    ...
    parser.add_argument("--rpc", required=True, help="RPC 地址（用于 nonce/gas/签名）")
...
def main() -> None:
    ...
    gas_estimate = int(rpc.call(lambda w3: w3.eth.estimate_gas(tx_for_estimate)))
    gas_limit = max(21_000, int(gas_estimate * args.gas_multiplier))
