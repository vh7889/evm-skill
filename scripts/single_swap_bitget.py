...
def normalize_amount_to_balance(
    rpc: ResilientRPC, wallet_addr: str, from_contract: str, amount_text: str
) -> str:
    ...
    c = rpc.call(lambda w3: w3.eth.contract(address=token_addr, abi=abi))
    decimals = int(rpc.call(lambda _w3: c.functions.decimals().call()))
    balance_raw = int(rpc.call(lambda _w3: c.functions.balanceOf(Web3.to_checksum_address(wallet_addr)).call()))
...
def main() -> None:
    ...
    effective_amount = normalize_amount_to_balance(
        rpc, wallet_addr, args.from_contract, args.amount
    )
    ...
    if tx_value == 0 and args.from_contract.strip() == "":
        tx_value = int(rpc.call(lambda w3: w3.to_wei(Decimal(effective_amount), "ether")))

    nonce = int(rpc.call(lambda w3: w3.eth.get_transaction_count(wallet_addr, "pending")))
    tx = {
        "chainId": chain_id,
        "from": wallet_addr,
