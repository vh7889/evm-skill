...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="单钱包一对一转原生 Gas 代币")
    parser.add_argument("--private-key", required=True, help="转出钱包私钥")
    parser.add_argument("--to", required=True, help="接收地址")
    parser.add_argument("--amount", required=True, help="数量（人类可读，例如 0.0002）")
...
def main() -> None:
    ...
    amount_wei = int(rpc.call(lambda w3: w3.to_wei(Decimal(args.amount), "ether")))
